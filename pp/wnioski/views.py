...
from .forms import WniosekForm_All, WniosekForm_Przedmiot_New, WniosekForm_Przedmiot_Podglad, WniosekForm_Wnioskodawca, WniosekForm_New, WniosekForm_Szacujacy, WniosekForm_Kierownik_Dzialu_Nauki, WniosekForm_osoba_dokonujaca_opisu, WniosekForm_Kwestor, WniosekForm_Rektor, WniosekForm_Kierownik_Dzialu_Zamowien_Publicznych, PrzedmiotZamowieniaForm
...
def wniosek_moje(request):
	...
	przedmioty = Przedmiot_Zamowienia.objects.all().order_by('id')
	
	context_dict = {'wnioski': wnioski, 'przedmioty': przedmioty}
...
def przedmiot_podglad(request, mymodel_id):
	class MyModelForm(forms.ModelForm):
		class Meta:
			model = Przedmiot_Zamowienia

	model = get_object_or_404(Przedmiot_Zamowienia, pk=mymodel_id)
	
	current_user = request.user
	instance = get_object_or_404(Przedmiot_Zamowienia, pk=mymodel_id)
...
def przedmiot_new(request):
	if request.method == 'POST':
		form=WniosekForm_Przedmiot_New(request.POST)
		if form.is_valid():
			form.save()
				
			#return HttpResponseRedirect(reverse('pp:'))
		return  render(request, 'wniosek_generic.html', {
		'form': WniosekForm_Przedmiot_New(),
	})
